...
/** Build the full prompt sent to Replicate: style prefix + scene description */
function buildGenerationPrompt(
  ...
  const styleText =
    style === "Custom" && customStylePrompt ? customStylePrompt.trim() : style;
  return `${styleText} style: ${scenePrompt}`;
}
...
export function ProjectDetailPage() {
  ...
  const handleGenerateScene = useCallback(
    async (scene: Scene) => {
      ...
      // Build the full prompt including style so scenes are visually consistent
      const currentProject = project;
      const currentStyle = currentProject?.style ?? style;
      const currentCustomStyle =
        currentProject?.customStylePrompt ?? customStylePrompt;
    ...
    [actor, projectId, updateScene, project, style, customStylePrompt],
  );
  ...
  const handleMergeVideos = () => {
    ...
    const videoTags = doneScenes
      .map(
        (s, i) =>
          `<p style="color:#aaa;font-size:12px;margin:4px 0 2px">Scene ${i + 1}</p><video src="${s.videoUrl}" controls style="width:100%;max-width:900px;border-radius:8px;background:#000;margin-bottom:12px"><track kind="captions"/></video>`,
      )
      .join("");
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Merged Preview</title><style>body{background:#111;color:#fff;font-family:sans-serif;padding:24px;margin:0}h1{font-size:20px;margin-bottom:16px}</style></head><body><h1>🎬 Merged Video Preview</h1>${videoTags}</body></html>`;
    const blob = new Blob([html], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    window.open(url, "_blank");
    toast.success("Opening merged preview in new tab.");
  };

  const handleDownloadAll = () => {
    if (doneScenes.length === 0) return;
    toast.info(`Downloading ${doneScenes.length} clips...`);
    for (const [i, scene] of doneScenes.entries()) {
          ...
          <AnimatePresence mode="wait">
            {activeTab === "scenes" ? (
              ...
              >
                ...
                <div className="bg-muted/20 border border-border rounded-lg px-4 py-2.5 flex items-center gap-2 text-xs text-muted-foreground">
                  <Sparkles className="w-3.5 h-3.5 shrink-0 text-primary/70" />
                  <span>
                    Style applied to all scenes:{" "}
                    <span className="text-foreground font-medium">
                      {style === "Custom" && customStylePrompt
                        ? `Custom — "${customStylePrompt.slice(0, 60)}${customStylePrompt.length > 60 ? "…" : ""}"`
                        : style}
                    </span>
                  </span>
                </div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                {style === "Custom" && (
                  ...
                  >
                    <Textarea
                      placeholder='Describe your style... e.g. "Ghibli-inspired watercolor with warm golden tones and soft shadows"'
                      value={customStylePrompt}
                      onChange={(e) => setCustomStylePrompt(e.target.value)}
                      className="bg-input border-border text-foreground text-xs min-h-[80px] resize-none mt-1.5"
                      data-ocid="settings.textarea"
                    />
                    <p className="text-xs text-muted-foreground">
                      This text is prepended to every scene prompt.
                    </p>
                  </motion.div>
                )}
              </div>
        ...
        <div className="flex flex-col gap-1 shrink-0">
          ...
          {scene.status === "done" && (
            <Button
              ...
              data-ocid={`scenes.item.${index + 1}.secondary_button`}
            >
